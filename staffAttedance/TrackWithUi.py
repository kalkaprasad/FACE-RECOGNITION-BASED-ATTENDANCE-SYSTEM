...
import  requests
import  staffAttedance.checkconnection as connection
...
mycolor2 = '#2d3d5b'  # or use hex if you prefer
p1 = tk.PhotoImage(file='sbsicon.png')
url = 'http://sbsproject.kalkaprasad.com/index.php/Api/staffsaveattandance'
...
lbl = tk.Label(window, text="Face Recognition Based Attendance System (College Staff) ", bg="white", fg="black", width=50, height=3, font=('times', 30, 'italic bold'))
...
def savedattendance(Id,aa,date,timeStamp,status):
    urlgetdata = "http://sbsproject.kalkaprasad.com/index.php/Api/getstaffdetails?idno={}".format(Id)
    x = requests.get(urlgetdata)
    data = x.json()
    if(data=='[]'):
        messagebox.showerror("Error","You are Not registered! Please Register yourself")
    else:
        email=data[0]['email']
        print("id", Id)
        print("name", aa)
        ...
        myobj = {'idno': Id, 'name': aa, 'email': email,
