...
import  random
import StudentAttendance.checkconnection as connecton
import  StudentAttendance.htmlemail as sendnotifi
...
window = tk.Tk()
window.title("Shaheed Bhagat Singh State technical Campus B.tech CSE 2027-2021")
...
# this is for name
rollno = tk.Label(window, text=" Enter Roll No", width=20 , height=2 , fg="black" , bg="white", font=('times', 15, ' bold ') )
...
fatheremail = tk.Label(window, text="Father Email ", width=20 , fg="black", bg="white", height=2, font=('times', 15, ' bold '))
...
goToRegister = Button(window,text="register",fg='green',font = "Verdana 10 bold",command=lambda:studentdata())
...
def studentdata():
    if(str(rollnotxt1.get())==""):
        ...
        alertbox()
    elif(str(fatheremailtxt2.get())==""):
        alertbox()
    else:
        ranvalue = random.randint(0, 100000)
        passwords=str(nametxt2.get())+str(ranvalue)
        checks =senddata(str(rollnotxt1.get()),str(nametxt2.get()),str(emailtxt2.get()),str(phonetxt2.get()),str(fatheremailtxt2.get()),passwords)
        if(checks.text=="success"):
            sendnotifi.sendemail(str(emailtxt2.get()), passwords)
...
def senddata(roll,name,email,phone,fatheremail,passwords):
    today = date.today()
    # dd/mm/YY
    d1 = today.strftime("%d/%m/%Y")
    password=passwords
    print("d1 =", d1)
    myobj = {'rollno': roll, 'name': name, 'email': email, 'phone': phone,
             'fatheremail': fatheremail, 'password': password, 'date': d1}
